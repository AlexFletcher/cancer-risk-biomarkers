function [shannon_indices, simpson_indices, moranI_time_series,  gearyC_time_series, fpi0, fpi, fpi2, fpi3, mpi_pos, mpi_tot, lattice_erpos_time_series, lattice_ki67_time_series,...
    ...
    new_model2d_vary_probs(N_pos_mutations_for_cancer, s_pos, s_del, mut_prob, bx_size, time_series_to_save, weights_matrix, total_weight_sum, weights_matrix_bx, total_weight_sum_bx, prob_pos, prob_neut)
%UPDATED TO HAVE PROB POS AND PROB NEUT AS PARAMETERS WHEN CALLING FCN
...
% The following is the Gillespie algorithm:
    ...
    what_mutation_happens = rand;
    if (does_mutation_happen < prob_pos)
        what_mutation_happens = 1;
    elseif (does_mutation_happen < (prob_pos+ prob_neut))
                what_mutation_happens = 2;

    else
            what_mutation_happens = 3;
