function [shannon_indices, simpson_indices, moranI_time_series,  gearyC_time_series, fpi0, fpi, fpi2, fpi3, mpi_pos, mpi_tot, lattice_erpos_time_series, lattice_ki67_time_series,...
    ...
    new_model2d_non_overlapping_bx(N_pos_mutations_for_cancer, s_pos, s_del, mut_prob, bx_size, time_series_to_save, weights_matrix, total_weight_sum, weights_matrix_bx, total_weight_sum_bx)
...
prev_bx_locations = zeros(num_biopsies,3);
...
propen = [];
propen(1) = types_of_mutants(1,4) * (1/(((1 - s_del)^(types_of_mutants(1,1))) * ((1 + s_pos)^(types_of_mutants(1,3)))));
...
while (1) % we stop when the endpoint of cancer is reached
    ...
    if(next_biopsy_point <= num_biopsies)
        if(cur_time >= biopsy_time_points(next_biopsy_point))
            next_pt_diff = false;
            while(~next_pt_diff)
                done_choosing_pt=false;
                while(~done_choosing_pt)
                    biopsy_centre_x = randi(N_size- 2*biopsy_radius ) + (biopsy_radius);
                    biopsy_centre_y = randi(N_size- 2*biopsy_radius ) + (biopsy_radius);
                    for kk = 1:(next_biopsy_point-1)
                        if ((abs(prev_bx_locations(kk,1) - biopsy_centre_x)>= biopsy_radius) && (abs(prev_bx_locations(kk,2) -biopsy_centre_y)>= biopsy_radius))
                            done_choosing_pt=true;
                        else
                            if(prev_bx_locations(kk,3) ~= cur_time)
                                done_choosing_pt=true;
                            else
                                done_choosing_pt=false;
                                break;
                    ...
                    if(next_biopsy_point==1)
                        done_choosing_pt=true;
                    end
                end
                prev_bx_locations(next_biopsy_point,:) = [biopsy_centre_x biopsy_centre_y cur_time];
...
% The following is the Gillespie algorithm:
    ...
    propen(ind) = types_of_mutants(ind,4) * 1/(((1 - s_del)^(types_of_mutants(ind,1))) * ((1 + s_pos)^(types_of_mutants(ind,3))));
    ...
    if(mutation_occurring  >=1 )
        ...
        % mutation added
        new_cell_type = cell_lattice{cell_neighbour_x, cell_neighbour_y};
        new_cell_type(mutation_occurring) = new_cell_type(mutation_occurring)+ 1; % adds a  mutation
        cell_lattice{cell_loc_x,cell_loc_y} = new_cell_type;
        ...
        for k=1:length(types_of_mutants(:,1))
            if (isequal(types_of_mutants(k,1:3),new_cell_type))
                ...
                propen(k) = types_of_mutants(k,4) * 1/(((1 - s_del)^(types_of_mutants(k,1))) * ((1 + s_pos)^(types_of_mutants(k,3))));
                
                new_locs_list = cell_locations{new_cell_type(1) + 1, new_cell_type(2) + 1, new_cell_type(3) + 1};
                cell_locations{new_cell_type(1) + 1, new_cell_type(2) + 1, new_cell_type(3) + 1} = [new_locs_list; cell_loc_x cell_loc_y];
                break;
            end
        ...
        if (~foundflag)
            ...
            propen(temp_var) = types_of_mutants(temp_var,4) * 1/(((1 - s_del)^(types_of_mutants(temp_var,1))) * ((1 + s_pos)^(types_of_mutants(temp_var,3))));
            
            cell_locations{new_cell_type(1) + 1, new_cell_type(2) + 1, new_cell_type(3) + 1} = [cell_loc_x cell_loc_y];
            
        end
        
        if (old_cell_type(3) < N_pos_mutations_for_cancer && new_cell_type(3) ==N_pos_mutations_for_cancer)
        ...
        cell_lattice{cell_loc_x,cell_loc_y} = cell_lattice{cell_neighbour_x, cell_neighbour_y};
        new_cell_type = cell_lattice{cell_neighbour_x, cell_neighbour_y};
